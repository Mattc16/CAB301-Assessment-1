...
class Member : IMember
{
    ...
    public int CompareTo(IMember member)
    {
        ...
        int lastNameComparison = this.lastName.CompareTo(member.LastName);
        if (lastNameComparison != 0)
        {
            return lastNameComparison;
        }

        // if the last names are the same, compare based on first name
        int firstNameComparison = this.firstName.CompareTo(member.FirstName);
